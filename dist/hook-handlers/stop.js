...
var path4 = createPathTagFunction(encodeURIPath);
...
var BaseAnthropic = class {
  ...
  buildRequest(inputOptions, { retryCount = 0 } = {}) {
    ...
    const { method, path: path5, query } = options;
...
async function condenseIfNeeded(conversationId, summaryStore, config) {
  ...
  for (let i = 0; i + threshold <= uncondensed.length; i += threshold) {
    ...
    const pseudoMessages = batch.map((s, idx) => ({
      id: s.id,
    ...
    try {
      const { text, level: escalationLevel } = config.anthropicApiKey ? await summarizeWithEscalation(pseudoMessages, config.anthropicApiKey) : await summarizeWithCLIEscalation(pseudoMessages);
